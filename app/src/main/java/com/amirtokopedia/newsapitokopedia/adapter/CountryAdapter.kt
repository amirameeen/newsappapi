 ...
 */
class CountryAdapter(val context: Context, val data : CountryModel?) : RecyclerView.Adapter<CountryAdapter.MyViewHolder>() {
    ...
    inner class MyViewHolder(view: View) : RecyclerView.ViewHolder(view), View.OnClickListener {
        override fun onClick(v: View?) {
            if(v == itemView.menu_card){
                itemListener?.onItemSelectedCountry(data?.countries?.get(adapterPosition)!!, itemView.iv_check)
            }
        }
